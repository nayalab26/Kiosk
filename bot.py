...
async def save_user_chat_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Привет! Мы запомнили тебя и пришлём уведомление как только рассмотрим заявку. Обычно до 24 часов."
    )
...
async def process_approve(context, handle, message=None, query=None):
    ...
    if data and data[0].get('chat_id'):
        ...
        if message:
            await message.reply_text(f"Владелец не написал боту. Свяжитесь вручную: {data[0]['contact']}")
